...
func registerCompletionsRoute(
    _ app: Application, modelManager: ModelManager,
...
private struct TextCompletionParameters {
    ...
    init(from request: CompletionRequest) {
        ...
        self.repetitionPenalty =
            request.repetitionPenalty ?? GenerationDefaults.repetitionPenalty
        ...
        self.quantizedKVStart =
            request.quantizedKVStart ?? GenerationDefaults.quantizedKVStart
    }
}
        ...
        Task {
            ...
            do {
                ...
                for try await token in tokenStream {
                    ...
                    if let newTextChunk = detokenizer.next() {
                        ...
                        if let sseString = encodeSSE(
                            response: chunkResponse, logger: logger)
                        {
                ...
                if finalFinishReason == nil {
                    finalFinishReason =
                        (generatedTokens.count >= parameters.maxTokens)
                        ? "length" : "stop"
                }
            } catch {
                logger.error(
                    "Text stream error (ID: \(baseCompletionId)): \(error)")
            }
...
private func handleNonStreamingTextCompletion(
    ...
    do {
        ...
        logger.error(
            "Non-streaming text generation error (ID: \(baseCompletionId)): \(error)")
        throw ProcessingError(
            status: .internalServerError,
