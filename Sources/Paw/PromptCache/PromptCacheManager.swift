...
public actor PromptCacheManager {
    ...
    public func getCachedState(
        ...
        logger.info(
            "Cache hit: Reusing \(commonLength) tokens, processing \(tokens.count - commonLength) new tokens"
        )
    ...
    private func quantizeCachesIfNeeded(
        ...
        if quantizedCount > 0 {
            logger.info(
                "Quantized \(quantizedCount) KV caches with \(kvBits) bits, group size \(kvGroupSize)"
            )
        }
