...
actor ModelManager {
    ...
    init(defaultModelPath: String?, isVLM: Bool, logger: Logger) {
        ...
        self.modelDownloadBaseURL = documents.appendingPathComponent(
            "huggingface", isDirectory: true)

        logger.info(
            "ModelManager initialized. Default Model Path: \(defaultModelPath ?? "auto-detect"). Scanning Location: \(self.modelDownloadBaseURL.path)"
        )
    }

    func getModel(requestedModelId: String?) async throws -> (ModelContainer, Tokenizer, String) {
        let actualIdToLoad = try resolveModelId(requestedModelId)

        logger.debug(
            "getModel: Requested='\(requestedModelId ?? "nil (default)")', Resolved actualIdToLoad='\(actualIdToLoad)'"
        )
    ...
    private func resolveModelId(_ requestedModelId: String?) throws -> String {
        if let reqId = requestedModelId, reqId.lowercased() != "default_model" {
            return reqId
        }

        if let defaultPath = defaultModelPath {
            return defaultPath
        }

        // Auto-detect: find first available model
        let models = scanForAvailableModelsSync()
        guard let firstModel = models.first else {
            throw ModelManagerError(
                status: .internalServerError,
                reason:
                    "No default model specified and no models found in \(modelDownloadBaseURL.path)"
            )
        }

        logger.info("Auto-detected first available model: \(firstModel)")
        return firstModel
    }

    private func getCachedModelIfAvailable(_ actualIdToLoad: String) async -> (
        ModelContainer, Tokenizer, String
    )? {
        guard let currentContainer = currentModelContainer,
            actualIdToLoad == currentModelIdString,
            let loadedName = currentLoadedModelName
        else {
            return nil
        }

        logger.info(
            "Returning cached model: \(loadedName) (ID: \(currentModelIdString ?? "unknown"))")
    ...
    private func unloadCurrentModelIfNeeded(_ actualIdToLoad: String) async {
        ...
        logger.info(
            "Request for model to load '\(actualIdToLoad)'. Current loaded is '\(currentModelIdString ?? "none")'. Needs loading/reloading."
        )

        let unloadedModelId = currentModelIdString ?? "unknown"
        let unloadedModelName = currentLoadedModelName ?? "unknown"
        logger.info(
            "Unloading previous model: \(unloadedModelName) (ID: \(unloadedModelId))")
    ...
    private func loadNewModel(_ actualIdToLoad: String, requestedModelId: String?) async throws
        -> (ModelContainer, Tokenizer, String)
    {
        let (modelConfiguration, nameToReturnForThisLoad) = try createModelConfiguration(
            actualIdToLoad)

        do {
            logger.info(
                "Attempting to load: \(nameToReturnForThisLoad) using configuration for \(actualIdToLoad)"
            )
            let newContainer = try await modelFactory.loadContainer(
            ...
            logger.info(
                "Successfully loaded model: \(nameToReturnForThisLoad) (from ID/path: \(actualIdToLoad))"
            )
            return (newContainer, newTokenizer, nameToReturnForThisLoad)
        } catch {
            logger.error(
                "Failed to load model with ID/path '\(actualIdToLoad)' (resolved from request '\(requestedModelId ?? "default")'): \(error)"
            )
    ...
    private func createModelConfiguration(_ actualIdToLoad: String) throws -> (
    ...
    private func scanForAvailableModels() async -> [String] {
        return scanForAvailableModelsSync()
    }

    private func scanForAvailableModelsSync() -> [String] {
        logger.info("Scanning for available MLX models in \(modelDownloadBaseURL.path)...")
        var foundModelIds = Set<String>()
        let modelsRootURL = modelDownloadBaseURL.appendingPathComponent(
        ...
        logger.info(
            "Scan complete. Found potential models from \(modelDownloadBaseURL.path): \(sortedModels)"
        )
        return sortedModels
    }

    private func scanOrganizationDirectory(
        _ orgDirURL: URL, foundModelIds: inout Set<String>
    ) throws {
        ...
        for modelNameDirURL in modelNameDirs {
            ...
            if isMLXModelDirectory(modelNameDirURL) {
                ...
                logger.debug(
                    "Directory \(modelNameDirURL.path) is not a valid MLX model.")
            }
        }
    }
    ...
    private func isMLXModelDirectory(_ dirURL: URL) -> Bool {
        ...
        let tokenizerExists =
            fileManager.fileExists(atPath: requiredFiles.tokenizerPath)
            || fileManager.fileExists(atPath: requiredFiles.tokenizerModelPath)
        let weightsExist = checkForModelWeights(
            in: dirURL, indexPath: requiredFiles.indexPath)

        guard configExists && tokenizerExists && weightsExist else {
            logMissingFiles(
                for: dirURL, config: configExists, tokenizer: tokenizerExists,
                weights: weightsExist)
            return false
        }
    ...
    private func checkForModelWeights(in dirURL: URL, indexPath: String) -> Bool {
        ...
        do {
            ...
            logger.error(
                "Error checking for weights files in \(dirURL.path): \(error)")
            return false
        }
    }
